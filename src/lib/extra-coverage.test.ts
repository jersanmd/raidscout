...
function makeDeath(overrides: Partial<DeathRecord> = {}): DeathRecord {
  return {
    ...
    death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString(),
...
describe("fixed_hours — multiple kills & edge cases", () => {
  it("two kills 8h apart → second spawn 24h after second kill", () => {
    ...
    // Kill 1: noon
    const d1 = makeDeath({ id: "d1", death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    // 20h later (8am next day): still countdown
    let r = calculateSpawnInfo(boss, d1, new Date(Date.UTC(2025,5,2,8,0,0)));
    expect(r.status).toBe("countdown");
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,12,0,0)));

    // Boss spawns at noon, kill again at 8pm
    const d2 = makeDeath({ id: "d2", boss_id: "boss-1",
      death_time: new Date(Date.UTC(2025,5,2,20,0,0)).toISOString() });
    r = calculateSpawnInfo(boss, d2, new Date(Date.UTC(2025,5,2,20,1,0)));
    expect(r.status).toBe("countdown");
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,3,20,0,0)));
  });

  it("10h boss: killed at midnight, spawns at 10am", () => {
    const boss = makeBoss({ respawn_hours: 10 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,2,0,0,0)).toISOString() });
    const r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,2,9,59,0)));
    expect(r.status).toBe("countdown");
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,10,0,0)));
  });

  it("62h boss: killed Friday noon, spawns Monday 2am", () => {
    const boss = makeBoss({ respawn_hours: 62 });
    // June 6, 2025 = Friday
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,6,12,0,0)).toISOString() });
    // Sunday 10pm = 58h later, still countdown
    let r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,8,22,0,0)));
    expect(r.status).toBe("countdown");
    // Monday 2am exactly
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,9,2,0,0)));

    // Monday 2:01am = alive
    r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,9,2,1,0)));
    expect(r.status).toBe("alive");
  });

  it("reports remainingMs correctly for countdown", () => {
    const boss = makeBoss({ respawn_hours: 10 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
...
describe("fixed_schedule — day-of-week & slot windows", () => {
  it("multi-slot boss: alive after first slot, countdown after kill", () => {
    ...
    const now = new Date(Date.UTC(2025,5,2,19,30,0));
    let r = calculateSpawnInfo(boss, null, now);
    expect(r.status).toBe("alive");

    // Kill at Monday 8pm
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,2,20,0,0)).toISOString() });
    r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,2,20,1,0)));
    expect(r.status).toBe("countdown");
    // Next slot: Thursday 7pm
    expect(r.nextSpawn).not.toBeNull();
    const thuSlot = r.nextSpawn!;
    expect(thuSlot.getUTCDay()).toBe(4); // Thursday
    expect(thuSlot.getUTCHours()).toBe(19);
  });
  ...
  it("boss with 2 slots on same day — earlier slot alive, later slot countdown after kill", () => {
    ...
    let r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,4,12,0,0)));
    expect(r.status).toBe("alive");

    // Kill at 12:30pm — countdown to 7pm slot
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,4,12,30,0)).toISOString() });
    r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,4,12,31,0)));
    expect(r.status).toBe("countdown");
    expect(r.nextSpawn!.getUTCHours()).toBe(19);
  });
  ...
  it("Sunday slot: alive Sunday evening, countdown after 4h cap Monday morning", () => {
    ...
    let r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,1,22,30,0)));
    expect(r.status).toBe("alive");

    // Monday 3am — 5h after slot, past 4h cap → countdown
    r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,3,0,0)));
    expect(r.status).toBe("countdown");
  });
  ...
  it("returns countdown when now is before the first slot of the week", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,12,0,0)));
    expect(r.status).toBe("countdown");
    expect(r.nextSpawn!.getUTCDay()).toBe(5);
  });
  ...
  it("alive window closes 1h before next slot", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,17,59,0)));
    expect(r.status).toBe("countdown");
  });
  ...
  it("single-slot boss killed within alive window → countdown to next week", () => {
    ...
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,2,19,30,0)).toISOString() });
    const r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,2,19,31,0)));
    expect(r.status).toBe("countdown");
    // Next spawn = next Monday
    expect(r.nextSpawn!.getUTCDay()).toBe(1);
    expect(r.nextSpawn!.getUTCDate()).toBe(9); // one week later
  });
  ...
  it("single-slot boss alive window capped — Wednesday is countdown, not alive from last Saturday", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,4,12,0,0))); // June 4 = Wednesday
    expect(r.status).toBe("countdown");
  });
  ...
  it("single-slot boss still alive within 4h window of last-slot fallback", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,1,0,0))); // Monday 1am
    expect(r.status).toBe("alive");
  });
  ...
  it("single-slot boss countdown after 4h window expires", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,3,0,0))); // Monday 3am
    expect(r.status).toBe("countdown");
  });
  ...
  it("multi-slot boss alive window still works normally (not affected by cap)", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,2,22,0,0)));
    expect(r.status).toBe("alive");
  });
  ...
  it("multi-slot boss not alive at midnight when 4h cap expires", () => {
    ...
    const r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,3,2,0,0)));
    expect(r.status).toBe("countdown");
  });
});
...
describe("leaderboard — period boundaries", () => {
  ...
  it("weekly period starts on Monday 00:00", () => {
    ...
    const wed = new Date(Date.UTC(2025,5,4,15,30,0));
    const start = getWeekStart(wed);
    expect(start.getUTCDay()).toBe(1); // Monday
    expect(start.getUTCDate()).toBe(2); // June 2
    expect(start.getUTCHours()).toBe(0);
  });

  it("weekly period: Sunday is in the current week (starts previous Monday)", () => {
    // Sunday June 8, 2025
    const sun = new Date(Date.UTC(2025,5,8,12,0,0));
    const start = getWeekStart(sun);
    expect(start.getUTCDate()).toBe(2); // Monday June 2
  });

  it("weekly period: Monday itself is the start", () => {
    const mon = new Date(Date.UTC(2025,5,2,8,0,0));
  ...
  it("monthly period starts on the 1st at 00:00", () => {
    const mid = new Date(Date.UTC(2025,5,15,12,0,0));
  ...
  it("points falling exactly on period boundary are included", () => {
    const start = new Date(Date.UTC(2025,5,2,0,0,0)); // Monday midnight
    const recordTime = new Date(Date.UTC(2025,5,2,0,0,0)); // exact same time
    expect(recordTime.getTime()).toBeGreaterThanOrEqual(start.getTime());
  });

  it("points before period boundary are excluded", () => {
    const start = new Date(Date.UTC(2025,5,2,0,0,0)); // Monday midnight
    const recordTime = new Date(Date.UTC(2025,5,1,23,59,59)); // 1 second before
...
describe("death record history — multi-kill scenarios", () => {
  it("history accumulates: kill1 → kill2, both preserved", () => {
    ...
    // Kill 1
    history.push(makeDeath({ id: "d1", death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() }));
    // Kill 2 (boss respawned and was killed again)
    history.push(makeDeath({ id: "d2", death_time: new Date(Date.UTC(2025,5,2,12,0,0)).toISOString() }));
  ...
  it("latest death record is used for spawn calculation", () => {
    const deaths = [
      makeDeath({ id: "old", death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() }),
      makeDeath({ id: "latest", death_time: new Date(Date.UTC(2025,5,2,12,0,0)).toISOString() }),
    ];
  ...
  it("editing a death time shifts the spawn window", () => {
    ...
    let r = calculateSpawnInfo(boss, death, new Date(Date.UTC(2025,5,1,12,1,0)));
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,12,0,0)));

    // Edit death time to 8pm → spawn shifts to June 2 at 8pm
    const editedDeath = makeDeath({ id: "death-1",
      death_time: new Date(Date.UTC(2025,5,1,20,0,0)).toISOString() });
    r = calculateSpawnInfo(boss, editedDeath, new Date(Date.UTC(2025,5,1,20,1,0)));
    expect(r.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,20,0,0)));
  });
  ...
  it("deleting a death record restores unknown state (fixed_hours)", () => {
    ...
    r = calculateSpawnInfo(boss, null, new Date(Date.UTC(2025,5,1,12,1,0)));
