...
function makeDeath(overrides: Partial<DeathRecord> = {}): DeathRecord {
  return {
    ...
    boss_id: "boss-1",
    user_id: "user-1",
    death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString(), // June 1, 2025 noon
...
describe("calculateSpawnInfo — fixed_hours", () => {
  it("returns unknown when there is no death record (not killed yet)", () => {
    ...
    const now = new Date(Date.UTC(2025,5,1,12,0,0));
  ...
  it("returns countdown when spawn is in the future", () => {
    ...
    const now = new Date(Date.UTC(2025,5,2,8,0,0)); // 20h after death, 4h before spawn

    const result = calculateSpawnInfo(boss, death, now);

    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,12,0,0)));
  ...
  it("returns alive when spawn time has passed", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 24 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,3,0,0,0)); // 36h after death
  ...
  it("returns alive at exact spawn time", () => {
    ...
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,1,22,0,0)); // exactly 10h later
...
describe("calculateSpawnInfo — fixed_schedule", () => {
  it("returns next schedule slot as countdown when boss hasn't spawned yet this week", () => {
    ...
    const now = new Date(Date.UTC(2025,5,2,12,0,0)); // June 2, 2025 (Monday)
...
describe("calculateSpawnInfo — edge cases", () => {
  ...
  it("handles 48h respawn correctly", () => {
    ...
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,3,8,0,0)); // 44h later, 4h to go

    const result = calculateSpawnInfo(boss, death, now);

    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,3,12,0,0)));
  });

  it("handles 62h respawn correctly", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 62 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,4,0,0,0)); // 60h later, 2h to go

    const result = calculateSpawnInfo(boss, death, now);

    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,4,2,0,0)));
  });
});
...
describe("calculateSpawnInfo — spawn overrides", () => {
  it("override death_time takes priority over death record death_time", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() }); // noon
    // death alone → spawns at 10pm. Override shifts death to 2am → should spawn at noon
    const override = { death_time: new Date(Date.UTC(2025,5,1,2,0,0)).toISOString() };
    const now = new Date(Date.UTC(2025,5,1,11,0,0)); // 11am, 1h before override-based spawn

    const result = calculateSpawnInfo(boss, death, now, override);

    expect(result.status).toBe("countdown");
    // spawn = override.death_time + 10h = 2am + 10h = noon
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,12,0,0)));
  });

  it("override makes boss appear alive even without any death record", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    // No death record at all — would normally be "unknown"
    const override = { death_time: new Date(Date.UTC(2025,5,1,0,0,0)).toISOString() }; // midnight
    const now = new Date(Date.UTC(2025,5,1,11,0,0)); // 11am, 1h after spawn

    const result = calculateSpawnInfo(boss, null, now, override);

    expect(result.status).toBe("alive");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,10,0,0)));
  });

  it("null override falls back to death record normally", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,1,18,0,0)); // 6pm, 4h before spawn

    const result = calculateSpawnInfo(boss, death, now, null);

    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,22,0,0)));
  });

  it("override on a boss with a recent kill shifts the spawn earlier", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    // Killed at 10am → normally spawns at 8pm
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,10,0,0)).toISOString() });
    // Override: pretend death was at 6am → spawns at 4pm instead of 8pm
    const override = { death_time: new Date(Date.UTC(2025,5,1,6,0,0)).toISOString() };
    const now = new Date(Date.UTC(2025,5,1,14,0,0)); // 2pm, 2h before override spawn

    const result = calculateSpawnInfo(boss, death, now, override);

    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,16,0,0))); // 4pm
    // Without override it would be 8pm
  });

  it("override on unknown boss with no death record and no respawn_hours stays unknown", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: null });
    const override = { death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() };
...
describe("calculateSpawnInfo — real-world flows", () => {
  it("mark alive → kill → countdown (Venatus 10h)", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    const now = new Date(Date.UTC(2025,5,1,12,0,0)); // noon
    ...
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,12,30,0)).toISOString() });
    result = calculateSpawnInfo(boss, death, now, null); // no override after kill
    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,22,30,0))); // 12:30 + 10h
  });
  ...
  it("kill → edit spawn earlier → countdown adjusts", () => {
    ...
    const death = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,10,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,1,14,0,0)); // 2pm

    let result = calculateSpawnInfo(boss, death, now, null);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,20,0,0))); // 8pm
  ...
  it("fixed-schedule boss ignores override (schedule always wins)", () => {
    ...
    const now = new Date(Date.UTC(2025,5,2,20,0,0)); // Monday 8pm — within alive window
    const override = { death_time: new Date(Date.UTC(2025,5,1,0,0,0)).toISOString() };
  ...
  it("kill twice in same day with edit between", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    // First kill at 2am
    const death1 = makeDeath({ death_time: new Date(Date.UTC(2025,5,1,2,0,0)).toISOString() });
    let now = new Date(Date.UTC(2025,5,1,3,0,0));
    let result = calculateSpawnInfo(boss, death1, now, null);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,12,0,0))); // noon

    // Edit spawn to 8am → override: death = 8am - 10h = 10pm prev day
    const override = { death_time: new Date(Date.UTC(2025,5,0,22,0,0)).toISOString() };
    result = calculateSpawnInfo(boss, death1, now, override);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,8,0,0))); // 8am (already past)

    // Second kill at 8:30am → override deleted
    now = new Date(Date.UTC(2025,5,1,8,30,0));
    const death2 = makeDeath({ death_time: now.toISOString() });
    result = calculateSpawnInfo(boss, death2, now, null);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,18,30,0))); // 6:30pm
    expect(result.status).toBe("countdown");
  });
  ...
  it("multiple kills accumulate in history correctly", () => {
    const boss = makeBoss({ spawn_type: "fixed_hours", respawn_hours: 10 });
    // Kill 1: 2am
    const death1 = makeDeath({ id: "d1", death_time: new Date(Date.UTC(2025,5,1,2,0,0)).toISOString() });
    // Kill 2: 12pm (latest)
    const death2 = makeDeath({ id: "d2", death_time: new Date(Date.UTC(2025,5,1,12,0,0)).toISOString() });
    const now = new Date(Date.UTC(2025,5,1,13,0,0));

    // Spawn calculator uses latest death — should use death2
    const result = calculateSpawnInfo(boss, death2, now, null);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,22,0,0))); // 10pm
