...
function makeDeath(overrides: Partial<DeathRecord> = {}): DeathRecord {
  return {
    ...
    death_time: new Date(Date.UTC(2025,5,1,10,0,0)).toISOString(),
...
describe("Integration: full user session", () => {
  it("new server → mark alive → kill → kill again → edit spawn", () => {
    ...
    const override = { death_time: new Date(Date.UTC(2025,5,1,2,0,0)).toISOString() }; // now - 10h
    ...
    const kill1 = makeDeath({ id: "d1", death_time: new Date(Date.UTC(2025,5,1,12,30,0)).toISOString() });
    result = calculateSpawnInfo(boss, kill1, new Date(Date.UTC(2025,5,1,12,31,0)), null);
    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,22,30,0)));

    // --- Step 4: Wait 30 min, edit spawn to 2pm ---
    const now2 = new Date(Date.UTC(2025,5,1,13,0,0));
    const override2 = { death_time: new Date(Date.UTC(2025,5,1,4,0,0)).toISOString() }; // 2pm - 10h
    result = calculateSpawnInfo(boss, kill1, now2, override2);
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,14,0,0))); // 2pm

    // --- Step 5: Kill again at 2:15pm → override deleted ---
    const kill2 = makeDeath({ id: "d2", death_time: new Date(Date.UTC(2025,5,1,14,15,0)).toISOString() });
    const now3 = new Date(Date.UTC(2025,5,1,14,16,0));
    result = calculateSpawnInfo(boss, kill2, now3, null);
    expect(result.status).toBe("countdown");
    expect(result.nextSpawn).toEqual(new Date(Date.UTC(2025,5,2,0,15,0))); // next day 12:15am
  });
  ...
  it("two bosses: Venatus (10h) and Dalia (18h)", () => {
    ...
    // Mark all alive
    const vOverride = { death_time: new Date(Date.UTC(2025,5,1,2,0,0)).toISOString() }; // 12 - 10
    const dOverride = { death_time: new Date(Date.UTC(2025,5,0,18,0,0)).toISOString() }; // 12 - 18
    ...
    v = calculateSpawnInfo(venatus, vDeath, new Date(Date.UTC(2025,5,1,12,1,0)), null);
    expect(v.status).toBe("countdown");
    expect(v.nextSpawn).toEqual(new Date(Date.UTC(2025,5,1,22,0,0)));

    // Dalia should still be alive (different boss, override untouched)
    d = calculateSpawnInfo(dalia, null, new Date(Date.UTC(2025,5,1,12,1,0)), dOverride);
    expect(d.status).toBe("alive");
  });
  ...
  it("bulk mark alive: 3 unknown bosses → all become alive", () => {
    ...
    const overrides = [
      { boss_id: "b1", death_time: new Date(Date.UTC(2025,5,0,7,0,0)).toISOString() }, // 12 - 29
      { boss_id: "b2", death_time: new Date(Date.UTC(2025,5,0,15,0,0)).toISOString() }, // 12 - 21
      { boss_id: "b3", death_time: new Date(Date.UTC(2025,5,0,4,0,0)).toISOString() }, // 12 - 32
    ];
...
describe("Integration: fixed-schedule kill tracking", () => {
  it("killed after slot → not in alive window → countdown to next slot", () => {
    ...
    const now = new Date(Date.UTC(2025,5,2,20,30,0));
