...
export function AdminGamesTab() {
  ...
  const [editingItemId, setEditingItemId] = useState<string | null>(null); // non-null = edit mode in modal
  const [newItem, setNewItem] = useState({ name: "", rarity: "", description: "", category_id: "" as string | undefined, categoryLabel: "" as string | null, image_url: "" as string | undefined });
  ...
  const handleCreateItem = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      else if (editingItemId) imageUrl = newItem.image_url || undefined;
                ...
                {loadingTemplates ? (
                  ...
                  <>
                      ...
                      {TABS.map(t => {
                        ...
                        return (
                          <button key={t.key} onClick={() => setExpandedTab(t.key)}
                            ...
                            <Icon className="w-3.5 h-3.5" />{t.label}<span className={`text-[11px] ml-0.5 ${isActive ? "text-[#a1a1aa]" : "text-[#52525b]"}`}>({counts[t.key]})</span>
                          </button>
                        );
                      })}
                    </div>
                    <div className="px-4 py-3">
                      ...
                      {expandedTab === "items" && (
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2"><h4 className="text-xs font-semibold text-[#d4d4d8]">Item Catalog ({itemTotal[game.id] ?? (itemCatalog[game.id]||[]).length})</h4><div className="flex items-center gap-2"><div className="relative w-48"><Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-[#52525b]"/><input placeholder="Search items…" value={itemSearch} onChange={e=>setItemSearch(e.target.value)} className="w-full pl-7 pr-2 py-1.5 text-xs bg-[#0d0d11] border border-[#1e1e2a] rounded text-[#fafafa] placeholder:text-[#52525b] focus:outline-none focus:border-[#52525b]"/></div><button onClick={()=>{const gameRarities=rarities[game.id]||[];setShowAddItem(true);setEditingItemId(null);setNewItem({name:"",rarity:gameRarities[0]?.name||"",description:"",category_id:"",categoryLabel:"",image_url:""});setNewItemParent("");setItemImage(null);setItemImagePreview(null)}} className="flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-[#1e1e2a] hover:bg-[#2a2a35] text-[#d4d4d8] transition shrink-0"><Plus className="w-3 h-3"/> Add Item</button></div></div>
                                                    {showAddItem && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { setShowAddItem(false); setEditingItemId(null); setItemImage(null); setItemImagePreview(null); setNewItemParent(""); }}>
                              <div className="bg-[#0d0d11] border border-[#1e1e2a] rounded-xl p-5 w-full max-w-sm mx-4" onClick={e => e.stopPropagation()} onPaste={async () => { try { const items = await navigator.clipboard.read(); for (const item of items) { const imageType = item.types.find(t => t.startsWith('image/')); if (imageType) { const blob = await item.getType(imageType); const file = new File([blob], 'pasted-image.png', { type: blob.type }); setItemImage(file); setItemImagePreview(URL.createObjectURL(file)); break; } } } catch {} }}>
                                <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-semibold text-[#fafafa]">{editingItemId ? "Edit Item" : "Add Item"}</h3><button onClick={() => { setShowAddItem(false); setEditingItemId(null); setItemImage(null); setItemImagePreview(null); setNewItemParent(""); }} className="text-[#52525b] hover:text-[#fafafa]"><X className="w-4 h-4" /></button></div>
                                <div className="space-y-3">
                                  <div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Name</label><input value={newItem.name} onChange={e => setNewItem(p => ({...p, name: e.target.value}))} placeholder="e.g. Dragon Heart" className="w-full mt-1 px-3 py-2 bg-[#09090b] border border-[#1e1e2a] rounded-lg text-sm text-[#fafafa] placeholder:text-[#52525b] focus:outline-none focus:border-[#52525b]" autoFocus /></div>
                                  <div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Description (optional)</label><input value={newItem.description} onChange={e => setNewItem(p => ({...p, description: e.target.value}))} placeholder="Brief description" className="w-full mt-1 px-3 py-2 bg-[#09090b] border border-[#1e1e2a] rounded-lg text-sm text-[#fafafa] placeholder:text-[#52525b] focus:outline-none focus:border-[#52525b]" /></div>
                                  <div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Category</label><select value={newItemParent} onChange={e => { const pid = e.target.value; const parentCat = (categories[game.id]||[]).find(c=>c.id===pid); setNewItemParent(pid); const hasSubs = (categories[game.id]||[]).some(c=>c.parent_id===pid); setNewItem(p=>({...p, category_id: pid&&!hasSubs?pid:undefined, categoryLabel: pid&&!hasSubs?parentCat?.name||null:null})); }} className="w-full mt-1 px-3 py-2 bg-[#09090b] border border-[#1e1e2a] rounded-lg text-sm text-[#fafafa] focus:outline-none focus:border-[#52525b]"><option value="">None</option>{(categories[game.id]||[]).filter(c=>!c.parent_id).map(cat=>(<option key={cat.id} value={cat.id}>{cat.name}</option>))}</select></div>
                                  {newItemParent&&(categories[game.id]||[]).some(c=>c.parent_id===newItemParent)&&(<div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Subcategory</label><select value={newItem.category_id||''} onChange={e => { const sid = e.target.value; const subCat = (categories[game.id]||[]).find(c=>c.id===sid); const parentCat = (categories[game.id]||[]).find(c=>c.id===newItemParent); setNewItem(p => ({...p, category_id: sid || undefined, categoryLabel: subCat ? (parentCat?`${parentCat.name} → ${subCat.name}`:subCat.name) : null})); }} className="w-full mt-1 px-3 py-2 bg-[#09090b] border border-[#1e1e2a] rounded-lg text-sm text-[#fafafa] focus:outline-none focus:border-[#52525b]"><option value="">-- Select --</option>{(categories[game.id]||[]).filter(c=>c.parent_id===newItemParent).map(sub=><option key={sub.id} value={sub.id}>{sub.name}</option>)}</select></div>)}
                                  <div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Image (optional)</label>{itemImagePreview ? (<div className="mt-1 relative rounded-lg overflow-hidden bg-[#09090b] border border-[#1e1e2a]"><img src={itemImagePreview} alt="Preview" className="w-full h-32 object-contain" /><button onClick={() => { setItemImage(null); setItemImagePreview(null); setNewItem(p => ({...p, image_url: ""})); }} className="absolute top-1 right-1 p-1 rounded-full bg-black/60 text-[#fafafa] hover:bg-black/80 transition"><X className="w-3.5 h-3.5" /></button></div>) : (<div className={`mt-1 border-2 border-dashed rounded-lg p-4 text-center transition cursor-pointer ${imageDragOver ? 'border-[#52525b] bg-[#1e1e2a]/50' : 'border-[#1e1e2a] hover:border-[#3f3f46]'}`} onDragOver={e => { e.preventDefault(); setImageDragOver(true); }} onDragLeave={() => setImageDragOver(false)} onDrop={e => { e.preventDefault(); setImageDragOver(false); const f = e.dataTransfer.files[0]; if (f) { setItemImage(f); setItemImagePreview(URL.createObjectURL(f)); } }} onClick={() => document.getElementById('admin-item-image')?.click()}><Upload className="w-5 h-5 text-[#52525b] mx-auto mb-1" /><p className="text-[11px] text-[#52525b]"><span className="text-[#71717a] font-medium">Click to upload</span> or drag &amp; drop</p><p className="text-[11px] text-[#52525b]/50 mt-0.5">or <kbd className="px-1 py-0.5 rounded bg-[#1e1e2a] text-[#71717a] text-[11px]">Ctrl+V</kbd> paste from clipboard</p></div>)}<input id="admin-item-image" type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) { setItemImage(f); setItemImagePreview(URL.createObjectURL(f)); } e.target.value = ''; }} /></div>
                                  <div><label className="text-[11px] text-[#71717a] uppercase tracking-wider">Rarity</label><div className="flex gap-1.5 mt-1">{(rarities[game.id]||[]).map(r => (<button key={r.id} onClick={() => setNewItem(p => ({...p, rarity: r.name}))} className="flex-1 py-1.5 rounded-md text-[11px] font-medium capitalize transition border" style={{ borderColor: newItem.rarity===r.name ? r.color : '#27272a', color: newItem.rarity===r.name ? r.color : '#52525b', backgroundColor: newItem.rarity===r.name ? r.color+'15' : 'transparent' }}>{r.name}</button>))}{(rarities[game.id]||[]).length===0 && <span className="text-[11px] text-[#52525b] py-1">No rarities defined</span>}</div></div>
                                  <button onClick={handleCreateItem} disabled={submittingItem || !newItem.name.trim()} className="w-full py-2 bg-[#fafafa] text-[#09090b] rounded-lg text-sm font-medium hover:bg-[#e4e4e7] transition disabled:opacity-50">{submittingItem ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : editingItemId ? 'Save Changes' : 'Add Item'}</button>
                                </div>
                              </div>
                            </div>
                          )}
                          <div className="space-y-1">{(itemCatalog[game.id]||[]).map((it:ItemCatalogItem)=>{const rarityObj=(rarities[game.id]||[]).find(r=>r.name===it.rarity);const rarityColor=rarityObj?.color||"#71717a";const cat=(categories[game.id]||[]).find(c=>c.id===it.category_id);const catParent=cat?.parent_id?(categories[game.id]||[]).find(c=>c.id===cat.parent_id):null;const catLabel=cat?(catParent?`${catParent.name} → ${cat.name}`:cat.name):null;return(<div key={it.id} className="bg-[#0d0d11]/30 rounded overflow-hidden"><div className="flex items-center justify-between px-3 py-2 text-sm"><div className="flex items-center gap-2 min-w-0">{it.image_url?<img src={it.image_url} alt={it.name} className="w-5 h-5 rounded object-cover border border-[#1e1e2a]" style={{backgroundColor:rarityColor+"20"}}/>:<Package className="w-4 h-4 text-[#52525b]"/>}<span className="text-[#fafafa] truncate">{it.name}</span><span className="text-[11px] px-1.5 py-0.5 rounded font-medium shrink-0" style={{backgroundColor:rarityColor+"20",color:rarityColor,border:`1px solid ${rarityColor}40`}}>{it.rarity}</span>{catLabel&&<span className="text-[11px] text-[#52525b] truncate hidden sm:inline">{catLabel}</span>}</div><div className="flex items-center gap-1 shrink-0"><button onClick={()=>{setShowAddItem(true);setEditingItemId(it.id);setNewItem({name:it.name,rarity:it.rarity,description:it.description||"",category_id:it.category_id||"",categoryLabel:catLabel||"",image_url:it.image_url||""});if(cat?.parent_id){setNewItemParent(cat.parent_id)}else if(it.category_id){setNewItemParent(it.category_id)}else{setNewItemParent("")};if(it.image_url){setItemImagePreview(it.image_url);setItemImage(null)}else{setItemImage(null);setItemImagePreview(null)}}} className="p-1 text-[#52525b] hover:text-[#d4d4d8]"><Pencil className="w-3 h-3"/></button><button onClick={()=>setDeleteConfirm({type:"item",id:it.id,name:it.name,gameName:game.name})} className="p-1 text-[#52525b] hover:text-[#f87171]"><Trash2 className="w-3 h-3"/></button></div></div></div>)})}
                                ...
                                >
                                  <div className="flex items-center justify-between px-3 py-2 text-sm">
                                    <div className="flex items-center gap-2">
                                      <span className="text-[11px] text-[#52525b] w-4 text-right tabular-nums">{slot.sort_order}</span>
                                      <Shield className="w-3.5 h-3.5 text-[#52525b]" />
                                      <span className="text-[#fafafa]">{slot.name}</span>
                                      <span className="text-[11px] text-[#52525b]">({cats.length} categories)</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                  ...
                                  {cats.length > 0 && (
                                    <div className="border-t border-[#1e1e2a] px-3 py-1.5 flex items-center gap-1.5 flex-wrap">
                                      {cats.map((cat: any) => (
                                        <span key={cat.id} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[#1e1e2a] text-[11px] text-[#d4d4d8]">
