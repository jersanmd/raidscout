          ...
          <div key={game.id} className="bg-slate-800/30 border border-slate-700/50 rounded-lg overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3">
              <button onClick={() => toggleGame(game.id)} className="flex items-center gap-3 flex-1 text-left">
                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-purple-500/20 overflow-hidden">
                  {game.icon_url ? (
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {(bossTemplates[game.id] || []).map((bt: BossTemplate) => {
                          ...
                          return (
                          <div key={bt.id} className="bg-slate-900/30 rounded overflow-hidden">
                            ...
                            <div className="flex items-center gap-1">
                              <button onClick={() => setEditingBoss(isEditing ? null : { id: bt.id, name: bt.name, spawn_type: bt.spawn_type, respawn_hours: bt.respawn_hours, schedule: bt.schedule, is_recurring: bt.is_recurring, points: bt.points, category: bt.category, tags: bt.tags, image_url: bt.image_url })} className="p-1 text-slate-600 hover:text-slate-300"><Pencil className="w-3 h-3" /></button>
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {(activityTemplates[game.id] || []).map((at: ActivityTemplate) => {
                          ...
                          return (
                          <div key={at.id} className="bg-slate-900/30 rounded overflow-hidden">
                            <div className="flex items-center justify-between px-3 py-2 text-sm">
                            <div className="flex items-center gap-2">
                              {at.image_url ? (
                                <img src={at.image_url} alt={at.name} className="w-5 h-5 rounded object-cover border border-slate-700" />
                              ) : (
                                <Calendar className="w-4 h-4 text-slate-600" />
                              )}
                              <span className="text-white">{at.name}</span>
                              <span className="text-xs px-1.5 py-0.5 rounded bg-slate-700 text-slate-400">{at.schedule_type === "recurring" ? "Fixed Schedule" : at.schedule_type === "one_time" ? "Fixed Hours" : at.schedule_type}</span>
