...
export function ParticipantModal({
 ...
 {onChangeGuild && (
 <button onClick={onChangeGuild} className="text-xs text-[#a1a1aa]/80 hover:text-violet-300 transition px-2 py-1 rounded hover:bg-[#27272a]">
 Change Guild
 </button>
 )}
 {onEditDeathTime && (
 <button onClick={onEditDeathTime} className="text-xs text-[#a1a1aa]/80 hover:text-cyan-300 transition px-2 py-1 rounded hover:bg-[#27272a]">
 Edit Time
 </button>
 )}
 {onDelete && (
 <button onClick={onDelete} className="text-xs text-[#a1a1aa]/80 hover:text-red-300 transition px-2 py-1 rounded hover:bg-[#18181b]">
 Remove
 </button>
 )}
 <button onClick={onClose} className="text-[#71717a] hover:text-[#fafafa] transition p-1">
 <X className="w-4 h-4" />
 </button>
 </div>
 </div>
 ...
 {!readOnly && (
 <div>
 <p className="text-[10px] text-[#71717a] uppercase tracking-wider mb-1">Rally Screenshot (AI)</p>
 ...
 <img src={preview} alt={`Rally ${i + 1}`} className="w-16 h-16 object-cover rounded-lg border border-[#27272a] cursor-pointer hover:border-[#52525b] transition" onClick={() => setFullscreenPreviewIndex(i)} />
 <button onClick={(e) => { e.stopPropagation(); setRallyImages(prev => prev.filter((_, j) => j !== i)); setRallyPreviews(prev => prev.filter((_, j) => j !== i)); }} className="absolute -top-1.5 -right-1.5 p-0.5 rounded-full bg-red-600 text-[#fafafa] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition"><X className="w-3 h-3" /></button>
 </div>
 ))}
 <button onClick={() => fileInputRef.current?.click()} className="w-16 h-16 rounded-lg border-2 border-dashed border-[#27272a] text-[#71717a] hover:text-[#a1a1aa] hover:border-[#3f3f46] transition flex items-center justify-center"><Plus className="w-4 h-4" /></button>
 </div>
 {aiLoading && <span className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium bg-[#27272a] text-[#a1a1aa]"><Loader2 className="w-3 h-3 animate-spin" /> Scanning...</span>}
 {aiError && <p className="text-[10px] text-[#a1a1aa]">{aiError}</p>}
 ...
 <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border-2 border-dashed border-[#27272a] text-[#71717a] hover:text-[#a1a1aa] hover:border-[#3f3f46] transition text-xs"><ImagePlus className="w-3.5 h-3.5" />Upload rally screenshot for AI scan</button>
 )}
 </div>
 )}
 ...
 <div className="mb-3">
 <p className="text-[10px] text-[#71717a] uppercase tracking-wider mb-1">Rally Screenshots ({savedRallyUrls.length})</p>
 ...
 <p className="text-[10px] text-[#71717a] uppercase tracking-wider">Participants ({attendance.length})</p>
 </div>
 <input type="text" placeholder="Search members…" value={memberSearch} onChange={(e) => setMemberSearch(e.target.value)} className="w-full bg-[#18181b] border border-[#27272a] rounded-lg px-3 py-2 text-sm text-[#fafafa] placeholder-[#52525b] outline-none focus:border-[#52525b] focus:ring-1 focus:ring-[#27272a] transition mb-2" />
 <div className="max-h-96 overflow-y-auto rounded-lg border border-[#27272a] bg-[#18181b] p-2 space-y-2">
 {guildGroups.length === 0 ? (
 <p className="text-xs text-[#52525b] text-center py-3">No members found.</p>
 ) : (
 ...
 {!partyLeadersLoading && (
 readOnly ? (
 <span className={`text-xs px-2 py-1 rounded border font-medium ${group.color.text} ${group.color.border} bg-[#18181b]`}>Leader: {members.find(m => m.id === partyLeaders[group.guildId!])?.name || "—"}</span>
 ) : (
 <select
 ...
 <p className="text-[10px] font-medium text-[#71717a] uppercase tracking-wider mb-1 px-1">No Guild</p>
 )}
 {/* Member chips */}
 ...
 return (
 <label key={m.id} title={readOnly ? "Only moderators can update participants" : undefined} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded transition text-sm ${readOnly ? "cursor-default" : "cursor-pointer"} ${isAttending ? "bg-[#27272a] text-emerald-300 border border-[#27272a]" : "text-[#a1a1aa] hover:bg-[#27272a] border border-transparent"}`}>
 ...
 <button onClick={() => setFullscreenPreviewIndex(null)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 text-[#fafafa] hover:bg-white/20 transition z-10"><X className="w-6 h-6" /></button>
 <img
