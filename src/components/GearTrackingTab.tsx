...
const DEFAULT_TL_TEMPLATE = [
  { category: "Armor", slots: ["Helmet", "Armor", "Gloves", "Pants", "Shoes"] },
  {
    ...
    slots: ["Earrings", "Necklace", "Bracelet", "Ring", "Belt"],
  },
...
export function GearTrackingTab() {
  ...
  const [editingGear, setEditingGear] = useState<
    Record<
      string,
      Record<string, { itemId: string; enh: number; itemName?: string }>
    >
  >({});
  ...
  const [guildFilter, setGuildFilter] = useState<string>(() => {
    try {
      return localStorage.getItem(guildFilterKey) || "all";
    } catch {
      return "all";
    }
  });
  ...
  const [sortCol, setSortCol] = useState<string | null>(() => {
    ...
    try {
      const s = JSON.parse(localStorage.getItem(gearSortKey) || "null");
      return s?.col || null;
    } catch {
      return null;
    }
  ...
  const toggleSort = (col: string) => {
    ...
    setSortDir(newDir);
    try {
      localStorage.setItem(gearSortKey, JSON.stringify({ col, dir: newDir }));
    } catch {}
  };
  ...
  const sortMembers = (membersArr: Member[]): Member[] => {
    ...
    return [...membersArr].sort((a, b) => {
      ...
      const resolveRarity = (g: any) => {
        const item =
          g?.catalog_item ||
          itemCatalogItems.find((c: any) => c.id === g?.catalog_item_id) ||
          catalog.find((c) => c.id === g?.catalog_item_id);
  ...
  const [guildOrder, setGuildOrder] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem(guildOrderKey) || "[]");
    } catch {
      return [];
    }
  });
  ...
  const orderedGuilds = useMemo(() => {
    ...
    guilds.forEach((g) => {
      if (!ordered.includes(g.id)) ordered.push(g.id);
    });
    return ordered
  ...
  useEffect(() => {
    if (
      ...
      !localStorage.getItem(guildFilterKey)
    ) {
  ...
  const handleGuildFilterChange = (value: string) => {
    ...
    try {
      localStorage.setItem(guildFilterKey, value);
    } catch {}
  };
        ...
        if (gearSlots && gearSlots.length > 0) {
          ...
          return {
            slots: [
              {
                category: "Equipment",
                slots: gearSlots.map((s: any) => s.name),
              },
            ],
          };
        }
  ...
  const { data: memberGear = [], isPending: gearLoading } = useQuery<
    MemberGear[]
  >({
  ...
  const { data: gearSummaries = {} } = useQuery<Record<string, GearSummary>>({
    ...
    queryFn: async () => {
      const { data } = await supabase.rpc("get_gear_summary", {
        p_server_id: serverId,
      });
      const map: Record<string, GearSummary> = {};
      (data || []).forEach((r: GearSummary) => {
        map[r.member_id] = r;
      });
      return map;
    },
  ...
  const itemsBySlot = useMemo(() => {
    ...
    allSlotIds.forEach((slotName) => {
      ...
      if (catIds?.length) {
        map[slotName] = itemCatalogItems.filter((item: any) =>
          catIds.includes(item.category_id),
        );
      } else {
        map[slotName] = [];
      }
  ...
  const addCatalogItem = async () => {
    ...
    try {
      ...
      console.warn("[gear] audit write failed:", auditErr.message);
    }
    setNewItem({
      name: "",
      category: "",
  ...
  const saveMemberGear = async (memberId: string) => {
    ...
    try {
      ...
      for (const [slotId, { itemId, enh, itemName }] of Object.entries(
        changes,
      )) {
        ...
        if (existing) {
          ...
          if (!oldItemName && resolvedNewName) {
            ...
            oldItemName &&
            resolvedNewName &&
            oldItemName !== resolvedNewName
          ) {
            auditChanges.push(
              `${slotLabel}: ${oldItemName} → ${enh ? `+${enh} ` : ""}${resolvedNewName}`,
            );
          } else if (
            oldItemName &&
            oldItemName === resolvedNewName &&
            enh !== existing.enhancement_level
          ) {
            auditChanges.push(
              `${slotLabel}: ${resolvedNewName} +${existing.enhancement_level || 0} → +${enh || 0}`,
            );
          }
        } else {
          if (resolvedNewName) {
      ...
      if (auditChanges.length > 0 && serverId) {
        try {
          await writeAuditEntry({
            action: AuditAction.GEAR_EQUIP,
            server_id: serverId,
            target_id: memberId,
            details: { member_name: memberName, changes: auditChanges },
          });
        } catch (auditErr: any) {
          console.warn(
            "[gear] audit write failed, gear save succeeded:",
            auditErr.message,
          );
        }
      }
  ...
  const renderGearRow = (m: Member, idx?: number) => {
    ...
    return (
      <tr
        key={m.id}
        className="group border-b border-[#27272a]/30 hover:bg-[#09090b]/30 transition"
      >
        <td className="py-2 px-3 sticky left-0 bg-[#18181b] group-hover:bg-[#131316] z-10 transition-colors">
          <div className="flex items-center gap-1.5">
            {idx != null && (
              <span className="text-[11px] text-[#52525b] font-mono w-4 shrink-0 text-right">
                {idx + 1}
              </span>
            )}
          ...
          {m.class && (
            <span
              className="block text-[11px] ml-[22px] -mt-0.5"
              style={{ color: classColors[m.class] || "#71717a" }}
            >
              {m.class}
            </span>
          )}
        ...
        {allSlotIds.map((slotId) => {
          ...
          const rarityCol = item
            ? rarityColor(item.rarity?.toLowerCase()) || "#a1a1aa"
            : undefined;
          ...
          const handleSlotClick = () => {
            ...
            if (g?.catalog_item_id) {
              setEditingGear((prev) => ({
                ...
                [m.id]: {
                  ...(prev[m.id] || {}),
                  [slotId]: {
                    itemId: g.catalog_item_id!,
                    enh: g.enhancement_level || 0,
                    itemName: item?.name,
                  },
                },
              }));
            ...
            setTimeout(() => {
              gearEditorRef.current?.scrollIntoView({
                behavior: "smooth",
                block: "center",
              });
            }, 100);
          ...
          return (
            <td
              key={slotId}
              className={`py-1.5 px-2 text-center ${canManage ? "cursor-pointer hover:bg-[#09090b]/50 transition" : ""}`}
              onClick={handleSlotClick}
              title={canManage ? "Click to change item" : undefined}
            >
              {item ? (
                <div className="flex items-center justify-center">
                  <div
                    className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 relative"
                    style={{ backgroundColor: `${rarityCol}18` }}
                  >
                    {item.image_url ? (
                      <img
                        src={item.image_url}
                    ...
                    {enh > 0 && (
                      <span className="absolute right-0 bottom-1.5 text-[11px] font-black text-amber-400 bg-gradient-to-t from-black/20 to-transparent rounded-bl-lg rounded-tr-lg pl-1.5 pr-1 pt-1 pb-0.5 leading-none drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)]">
                        +{enh}
                      </span>
                    )}
                  </div>
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-[#71717a] text-sm">
          No gear template configured for this server.
        </p>
        {canManage && (
          <p className="text-[#52525b] text-xs mt-1">
            A default Throne &amp; Liberty template will be created
            automatically.
          </p>
        )}
      </div>
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div
          className={`px-3 py-2 rounded-lg text-xs ${toast.type === "success" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}
        >
          {toast.message}
          <button
            onClick={() => setToast(null)}
            className="ml-2 hover:text-[#fafafa]"
          >
                ...
                <div className="flex-1 flex items-center gap-2 bg-[#09090b] border border-[#27272a] rounded-lg px-3 py-2">
                      ...
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        {m.class && (
                          <span
                            ...
                            style={{ color: cc, backgroundColor: `${cc}18` }}
                          >
                            {m.class}
                          </span>
                        )}
                        {m.combat_power ? (
                          <span className="text-[11px] text-[#71717a] font-mono tabular-nums ml-auto">
                            {m.combat_power.toLocaleString()} CP
                          </span>
                        ) : null}
                      </div>
                    );
                  ...
                  <button
                    onClick={() => {
                      ...
                      setEditingGear((prev) => {
                        const next = { ...prev };
                        delete next[selectedMember];
                        return next;
                      });
                    }}
                    className="text-[#52525b] hover:text-[#fafafa] transition shrink-0"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-[#18181b] border border-[#27272a] rounded-lg max-h-64 overflow-y-auto z-10 shadow-xl">
                    {members
                      ...
                      .map((m) => {
                        ...
                        const CIcon = iconName ? getClassIcon(iconName) : null;
                        const cc =
                          (m.class && classColors[m.class]) || "#a1a1aa";
                        return (
                          <button
                            key={m.id}
                            onClick={() => {
                    ...
                    {members.filter((m) =>
                      ...
                      <p className="px-3 py-2 text-xs text-[#52525b]">
                        No members found
                      </p>
                    )}
                  </div>
              ...
              const initEditFromExisting = (slotId: string) => {
                ...
                if (existing && !edits[slotId]) {
                  setSlotEdit(
                    ...
                    existing.enhancement_level || 0,
                    existing.catalog_item?.name,
                  );
                }
              };
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {flatSlots.map((slotId) => {
                      ...
                      const currentItemId =
                        edit?.itemId ?? existing?.catalog_item_id ?? "";
                      const currentEnh =
                        edit?.enh ?? existing?.enhancement_level ?? 0;
                      const currentItem =
                        itemCatalogItems.find(
                          (c: any) => c.id === currentItemId,
                        ) || catalog.find((c) => c.id === currentItemId);
                      const rc = currentItem
                        ? rarityColor(currentItem.rarity?.toLowerCase()) ||
                          "#a1a1aa"
                        : undefined;
                      ...
                      return (
                        <button
                          key={slotId}
                          onClick={() => {
                            initEditFromExisting(slotId);
                            setOpenSlotPicker(isActive ? null : slotId);
                            setPickerSearch("");
                          }}
                          ...
                          {currentItem ? (
                            <div className="flex items-center gap-2.5">
                              ...
                              >
                                ...
                                <span className="absolute right-0 bottom-2 text-[11px] font-black text-amber-400 bg-gradient-to-t from-black/20 to-transparent rounded-bl-lg rounded-tr-lg pl-1.5 pr-1 pt-1 pb-0.5 leading-none drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)]">
                                  +{currentEnh}
                                </span>
                              </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-[11px] text-[#52525b] capitalize">
                                      {item.rarity}
                                    </p>
                                  </div>
                                </button>
          ...
          {selectedMember && (
            <div className="flex items-center justify-between pt-3 border-t border-[#27272a]">
              <div className="text-xs text-[#a1a1aa]">
                {gearSummaries[selectedMember] && (
                  <>
                    {gearSummaries[selectedMember].slots_filled}/
                    {gearSummaries[selectedMember].total_slots} slots
                  </>
                )}
              </div>
              <button
      ...
      {gearLoading ? (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-sm text-[#71717a]">
            Loading gear data...
          </span>
        </div>
      ) : (
        <>
          ...
          {orderedGuilds.length > 1 && (
            <div className="flex items-center gap-2 justify-end">
              <label className="text-[11px] text-[#71717a] tracking-wider">
                Filter By Guild:
              </label>
              <div className="relative">
                <button
                ...
                {guildFilterOpen && (
                  <>
                    <div
                      ...
                      onClick={() => setGuildFilterOpen(false)}
                    />
              ...
              >
                <div className="px-4 py-2 border-b border-[#27272a] flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-0.5 ml-auto">
                    <button
                      onClick={() => moveGuild(g.id, -1)}
                      disabled={gi === 0}
                      className="p-0.5 text-[#52525b] hover:text-[#fafafa] disabled:opacity-30 transition"
                    >
                      <ChevronUp className="w-3 h-3" />
                    </button>
                    <button
                      onClick={() => moveGuild(g.id, 1)}
                      disabled={gi === orderedGuilds.length - 1}
                      className="p-0.5 text-[#52525b] hover:text-[#fafafa] disabled:opacity-30 transition"
                    >
                    ...
                    <thead>
                      <tr className="text-[11px] text-[#71717a] uppercase tracking-wider border-b border-[#27272a]">
                        <th
                          ...
                          className="text-left py-2 px-3 sticky left-0 bg-[#18181b] z-10 cursor-pointer hover:bg-[#27272a]/30 transition select-none group"
                        >
                          <span
                            className={
                              sortCol === "name"
                        ...
                        <th
                          ...
                          className="text-center py-2 px-2 min-w-[60px] cursor-pointer hover:bg-[#27272a]/30 transition select-none group"
                        >
                          <span
                            className={
                              sortCol === "cp"
                        ...
                        {allSlotIds.map((slot) => (
                          <th
                            ...
                            className="text-center py-2 px-2 min-w-[80px] cursor-pointer hover:bg-[#27272a]/30 transition select-none group"
                          >
                            <span
                              className={
                                sortCol === slot
          ...
          {/* No-guild members */}
          {(guildFilter === "all" || guildFilter === "__noguild__") &&
            (guildMembers.get(null) || []).length > 0 && (
              <div className="bg-[#18181b] border border-[#27272a] rounded-xl overflow-hidden">
                <div className="px-4 py-2 border-b border-[#27272a] flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-[#52525b]">
                    {guildMembers.get(null)!.length} member
                    {guildMembers.get(null)!.length !== 1 ? "s" : ""}
                  </span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {sortMembers(guildMembers.get(null)!).map((m, i) =>
                        renderGearRow(m, i),
                      )}
                    </tbody>
                  </table>
