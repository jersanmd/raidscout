        ...
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-xl border border-neutral-800/70 bg-[#121316] p-6 space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] text-neutral-600 font-mono">or</span>
              <div className="flex-1 h-px bg-neutral-800/40" />
            </div>
            ...
            <div className="space-y-3">
              <p className="text-[11px] text-neutral-500 text-center">Claim your existing profile</p>
              ...
              {!claimsLoading && myClaims.length > 0 && (
                <div className="space-y-1">
                  <p className="text-[11px] text-neutral-600">Your Claims</p>
                  {myClaims.map(c => (
                    <div key={c.id} className="px-3 py-1.5 rounded bg-[#18191d]/40 border border-neutral-800/40 text-xs space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        {c.status === "pending" && <span className="text-[11px] text-amber-400 flex items-center gap-0.5 shrink-0"><Clock className="w-3 h-3" />Pending</span>}
                        {c.status === "accepted" && <span className="text-[11px] text-emerald-400 flex items-center gap-0.5 shrink-0"><CheckCircle className="w-3 h-3" />Accepted</span>}
                        {c.status === "declined" && <span className="text-[11px] text-red-400 flex items-center gap-0.5 shrink-0"><XCircle className="w-3 h-3" />Declined</span>}
                      </div>
                      {c.status === "declined" && c.decline_reason && (
                        <p className="text-[11px] text-neutral-500">Reason: {c.decline_reason}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <button
              onClick={signOut}
              className="flex items-center justify-center gap-1 text-[11px] text-neutral-600 hover:text-neutral-400 transition-colors duration-200 w-full"
            >
