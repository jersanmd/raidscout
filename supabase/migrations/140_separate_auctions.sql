-- 140: Separate dkp_auctions table for concurrent auctions of the same item
...
CREATE INDEX IF NOT EXISTS idx_dkp_auctions_item ON dkp_auctions(item_id, status);
CREATE INDEX IF NOT EXISTS idx_dkp_auctions_server ON dkp_auctions(server_id, status);

-- Add auction_id to dkp_bids (nullable for backward compat, existing bids get NULL)
ALTER TABLE public.dkp_bids ADD COLUMN IF NOT EXISTS auction_id UUID REFERENCES public.dkp_auctions(id) ON DELETE CASCADE;

-- ── Updated mark_item_for_bid: creates a new auction row each time ──
...
BEGIN
  ...
  -- Create one auction per quantity
  FOR v_i IN 1..GREATEST(p_quantity, 1) LOOP
    INSERT INTO public.dkp_auctions (item_id, server_id, dkp_cost, bid_end_time, guild_id)
  ...
  -- Keep items.is_up_for_bid in sync for backward compat
  UPDATE public.items SET is_up_for_bid = true WHERE id = p_item_id;

  -- Clear distributed status for new auctions
...
-- ── Updated place_bid: use auction_id ──
  ...
  IF v_prev_highest.id IS NOT NULL THEN
    ...
    IF v_outbid_user_id IS NOT NULL THEN
      ...
      VALUES (v_outbid_user_id, v_server_id, 'dkp_outbid',
        'You were outbid!',
        'Your bid of ' || v_prev_highest.bid_amount || ' DKP on "' || COALESCE(v_auction.item_name, 'Unknown item') || '" was outbid. Your DKP has been refunded.',
...
-- ── Updated resolve_auction: resolves a single auction ──
...
-- ── Updated auto_resolve_auction: resolves all active auctions for an item ──
...
DECLARE
  v_auction RECORD;
BEGIN
  FOR v_auction IN SELECT id FROM public.dkp_auctions WHERE item_id = p_item_id AND status = 'active'
  LOOP
    -- Use the single-auction resolve logic
    PERFORM public.resolve_auction(v_auction.id, NULL);
  END LOOP;
END;
$$;

-- ── Updated get_active_bids: include auction_id ──
