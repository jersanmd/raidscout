-- 164: Add item_name and rarity to DKP notification metadata
  ...
  IF v_prev_highest.id IS NOT NULL THEN
    ...
    IF v_outbid_user_id IS NOT NULL THEN
      ...
      VALUES (v_outbid_user_id, v_server_id, 'dkp_outbid',
        'You were outbid!',
        'The current bid on "' || COALESCE(v_auction.item_name, 'Unknown item') || '" is now ' || p_amount || ' DKP. Your ' || v_prev_highest.bid_amount || ' DKP has been refunded. Tap to bid again.',
        jsonb_build_object('auction_id', p_auction_id, 'outbid_amount', v_prev_highest.bid_amount, 'new_bid_amount', p_amount, 'item_name', v_auction.item_name, 'rarity', v_auction.item_rarity));
    END IF;
  END IF;
