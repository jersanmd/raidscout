-- Extend alive window from 4h to 24h for fixed_schedule bosses
...
BEGIN
  ...
  LOOP
    ...
    IF rec.spawn_type = 'fixed_hours' THEN
      ...
      ELSE v_spawn := '2025-01-01 00:00:00+00'::timestamptz; END IF;
    ELSIF rec.spawn_type = 'fixed_schedule' AND rec.schedule IS NOT NULL THEN
      ...
      IF v_recent IS NOT NULL AND v_last_death IS NOT NULL THEN
        ...
        IF v_next_slot IS NOT NULL THEN
          ...
          IF NOT v_was_killed AND v_now >= v_recent AND v_now < v_alive_until THEN v_spawn := v_recent;
          ELSE
            v_spawn := NULL;
            FOR d IN 0..7 LOOP
              check_day := (v_now AT TIME ZONE p_tz)::date + d;
