-- Randomize the auction soft-close extension.
--
-- Previously a bid placed with under 3 minutes left reset the deadline to exactly
-- now() + 3 minutes, so the close time was perfectly predictable and snipeable.
-- The extension is now a uniform random 180-300 seconds (inclusive), while the
-- trigger threshold stays at "under 180 seconds remaining".
--
-- Each qualifying bid rolls a fresh value, so repeated last-second bidding cannot
-- be timed. The frontend countdown reads bid_end_time and updates via the existing
-- realtime subscription on dkp_auctions, so no client change is needed -- viewers
-- will simply see the timer jump to somewhere between 3:00 and 5:00.
--
-- Everything else in place_bid is unchanged from 183_restore_bid_notifications.sql.
...
BEGIN
  ...
  -- random 180-300 seconds, so the exact close time cannot be sniped.
  v_remaining_secs := EXTRACT(EPOCH FROM (v_auction.bid_end_time - now()));
  IF v_remaining_secs < 180 THEN
    v_extend_secs := floor(random() * 121)::int + 180;   -- 180..300 inclusive
    UPDATE public.dkp_auctions
    SET bid_end_time = now() + make_interval(secs => v_extend_secs)
