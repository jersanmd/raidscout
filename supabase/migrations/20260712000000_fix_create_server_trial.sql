-- Fix: restore trial_ends_at in create_server_with_bosses
-- Migration 20260701000003 accidentally removed trial_ends_at from the INSERT,
-- causing all new servers to be born expired.
...
BEGIN
  ...
  IF v_guild_count = 1 THEN
    ...
    FROM public.bosses b WHERE b.server_id = v_server_id;

    INSERT INTO public.activity_guilds (activity_id, guild_id, sort_order, mode)
    SELECT a.id, v_guild_id, 1, 'rotation'
    FROM public.activities a WHERE a.server_id = v_server_id;
  END IF;

  -- Mark all bosses as alive (initial spawn)
  PERFORM public.bulk_mark_bosses_alive(v_server_id);
