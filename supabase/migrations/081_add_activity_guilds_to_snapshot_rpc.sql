-- Add activity_guilds to bot_server_snapshot RPC
-- Enables activity notifications with guild names in the spawn cron
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'activity_guilds', COALESCE((
      SELECT jsonb_agg(row_to_json(ag))
      FROM activity_guilds ag
      WHERE ag.guild_id IN (SELECT id FROM guilds WHERE server_id = p_server_id)
    ), '[]'::jsonb)
  )
  INTO v_result
  FROM servers s
