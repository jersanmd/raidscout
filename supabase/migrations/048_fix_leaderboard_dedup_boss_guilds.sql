-- 048: Fix get_leaderboard RPC — deduplicate boss_guilds per (boss_id, guild_id)
-- The unique index on boss_guilds is (boss_id, guild_id, COALESCE(day_of_week, -1)),
-- so a single boss+guild can have multiple rows with different day_of_week values.
-- The old LEFT JOIN public.boss_guilds would match ALL of them and SUM would
-- double/triple-count the per-guild point override. The edge function and
-- fetchMemberKills already deduplicate correctly.
-- Fix: use DISTINCT ON to pick the highest points value per (boss_id, guild_id).
