-- Remove seed.sql fallback from create_server_with_bosses — boss_templates are the only source now
...
-- Drop the legacy seed function (no longer called)
DROP FUNCTION IF EXISTS public.seed_bosses_for_server(UUID);
