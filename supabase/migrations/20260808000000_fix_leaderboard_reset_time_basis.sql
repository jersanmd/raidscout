-- Fix: get_leaderboard's "Since Reset" branch (p_since IS NULL) filtered boss kills by
-- attendance_records.created_at (when the row was written to Postgres) instead of
-- death_records.death_time (when the kill actually happened), while the explicit-p_since
-- branch used by Finalize correctly used death_time. Any attendance row added, corrected,
-- or re-copied after the guild's reset date counted on the live board even if the kill
-- itself happened (and was already paid out) before the reset -- causing live totals to
-- run higher than the Finalize snapshot by a different, inconsistent amount per member,
-- and risking the same kill's points being finalized twice if its attendance row is
-- touched again after its period was already finalized.
--
-- Also: point_adjustments never respected p_until, unlike boss_scores/activity_scores,
-- so adjustments made after a chosen Finalize cutoff (but before the button was clicked)
-- could leak into the snapshot. Added the same p_until bound here for consistency.
